 ...
 *    root and height atomically using cmpxchg16b in crfs_decrease_btree_height 
 ...
 * struct crfs_inode. It is used to implement the truncate linked list and is 
 * by crfs_truncate_add(), crfs_truncate_del(), and crfs_recover_truncate_list()
 * functions to manage the truncate list */
struct crfs_inode_truncate_item {
	__le64	i_truncatesize;     /* Size of truncated inode */
	__le64  i_next_truncate;    /* inode num of the next truncated inode */
};

/*
 * #define PMFS_NAME_LEN (PMFS_INODE_SIZE - offsetof(struct crfs_inode,
 *         i_d.d_name) - 1)
 */
 ...
 * crfs_get_block() returns NULL if the block offset is 0 (helps in catching
 ...
 * once the file system becomes stable and crfs_get_block() returns correct
 * pointers even for offset 0.
 */
struct crfs_super_block {
	...
	/* points to the location of crfs_journal_t */
	__le64          s_journal_offset;
	/* points to the location of struct crfs_inode for the inode table */
