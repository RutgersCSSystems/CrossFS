...
typedef struct crfs_transaction {
	u32              transaction_id;
	u16              num_entries;
	u16              num_used;
	u16              gen_id;
	u16              status;
	crfs_journal_t  *t_journal;
	crfs_logentry_t *start_addr;
	struct crfs_transaction *parent;
} crfs_transaction_t;

extern inline crfs_transaction_t *crfs_alloc_transaction(void);
extern inline void crfs_free_transaction(crfs_transaction_t *trans);

extern int crfs_journal_soft_init(struct super_block *sb);
extern int crfs_journal_hard_init(struct super_block *sb,
		uint64_t base, uint32_t size);
extern int crfs_journal_uninit(struct super_block *sb);
extern crfs_transaction_t *crfs_new_transaction(struct super_block *sb,
		int nclines);
extern inline crfs_transaction_t *crfs_current_transaction(void);
extern int crfs_add_logentry(struct super_block *sb,
		crfs_transaction_t *trans, void *addr, uint16_t size, u8 type);
extern int crfs_commit_transaction(struct super_block *sb,
		crfs_transaction_t *trans);
extern int crfs_abort_transaction(struct super_block *sb,
			crfs_transaction_t *trans);
extern int crfs_recover_journal(struct super_block *sb);
