...
#ifdef _NVSTATS
static unsigned int jemalloc_cflush_cntr;
#endif
...
static inline void clflush_range(volatile char* __p, size_t size)
{
	//fprintf(stdout,"clflush_range size %u\n", size);
#ifdef _USE_JEALOC_PERSISTONLY
	size_t count = 0;
	while(count < size) {
		asm volatile ("clflush %0\n" : "+m" (*(char *)(__p)));
