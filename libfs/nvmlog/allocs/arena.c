...
//NVRAM changes
static arena_run_t *nv_arena_run_alloc(arena_t *arena, size_t size, bool large,
		size_t binind, bool zero, void *rqst);
...
arena_run_reg_alloc(arena_run_t *run, arena_bin_info_t *bin_info)
{
	...
	clflush_range((volatile char *)run,sizeof(arena_run_t));
	//fprintf(stdout,"cache flush at arena_run_reg_alloc\n");
	//clflush_range((volatile char *)&run->nfree,sizeof(unsigned));
	//clflush_range((volatile char *)&run->nextind,sizeof(unsigned));
...
arena_run_reg_dalloc(arena_run_t *run, void *ptr)
{
	...
	//regind indicates bit in the bin (or) a corresponding malloc
...
#ifdef _USE_JEALOC_PERSISTONLY
	if(chunk )
	clflush_range((volatile char*)chunk, sizeof(arena_chunk_t));
#endif
}
...
arena_chunk_alloc(arena_t *arena)
{
	...
	rqst = (struct rqst_struct *)arena->rqst;
#endif
	...
	//fprintf(stdout,"arena_chunk_alloc\n");
	clflush_range((volatile char *)chunk,sizeof(arena_chunk_t));
	return (chunk);
}
		...
		bool zero)
{
	...
	if (mapelm != NULL) {
		...
		//fprintf(stdout,"flush at arena_run_alloc_helper \n");
		//clflush_range((volatile char *)run,sizeof(arena_run_t));
...
#ifdef _USENVRAM
		if(rqst && run ) {
			//fprintf(stdout, "rqst->mmapobj_straddr %lu \n",rqst->mmapobj_straddr);
			//run->map_strt_addr = rqst->mmapobj_straddr; 
		}
#endif
		//fprintf(stdout,"flush at arena_run_alloc\n");
		//clflush_range((volatile char *)run,sizeof(arena_run_t));
		return (run);
	}
		...
		uint64_t prof_accumbytes)
{
	...
	//fprintf(stdout,"flush at arena_tcache_fill_small\n");
	clflush_range((volatile char *)bin,sizeof(arena_bin_t));
	clflush_range((volatile char *)run,sizeof(arena_run_t));
}
...
arena_alloc_junk_small(void *ptr, arena_bin_info_t *bin_info, bool zero)
{
	...
	fprintf(stdout,"flush at arena_alloc_junk_small\n");
	clflush_range((volatile char *)bin_info,sizeof(arena_bin_info_t));
}
...
arena_malloc_small(arena_t *arena, size_t size, bool zero)
{
	...
	/*fprintf(stdout,"size: arena_run_t: %u "
					"size arena_chunk_map_t: %u "
					"size arena_bin_t: %u "
					"size arena_run_t: %u "
					"size arena_t: %u \n", sizeof(arena_run_t),
					 sizeof(arena_chunk_map_t), sizeof(arena_bin_t),
					 sizeof(arena_run_t), sizeof(arena_t));*/

	//clflush_range((volatile char *)bin,sizeof(arena_run_t));
	//clflush_range((volatile char *)arena,sizeof(arena_t));
	//fprintf(stdout,"flush at arena_malloc_small \n");
...
arena_malloc_large(arena_t *arena, size_t size, bool zero)
{
	...
	if (zero == false) {
     ...
     //fprintf(stdout,"flush at arena_malloc_large \n");
	//clflush_range((volatile char *)arena,sizeof(arena_t));
	return (ret);
}
		...
		size_t pageind, arena_chunk_map_t *mapelm)
{
	...
	fprintf(stdout,"flush at  arena_dalloc_bin \n");
	clflush_range((volatile char *)bin,sizeof(arena_bin_t));
}
...
arena_new(arena_t *arena, unsigned ind)
{
	...
	clflush_range((volatile char *)arena,sizeof(arena_bin_t));
