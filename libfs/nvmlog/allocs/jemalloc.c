...
//#ifndef _USENVRAM
//je_malloc_(size_t size)
//#else
je_malloc_(size_t size, rqst_s *rqst)
//#endif
{
	void *ret;
...
#ifdef _USENVRAM
//#ifndef _USE_JEALOC_PERSISTONLY
//#ifdef _USE_SHADOWCOPY
#ifdef _USE_TRANSACTION
	if(!rqst->no_dram_flg) {	
		/*rqst->log_ptr = malloc(logptr_sz+ (2*PAGE));
	    rqst->log_ptr = (void*)(((long)rqst->log_ptr+ PAGE-1) & ~(PAGE-1));
		logptr_sz = logptr_sz + (PAGE- (logptr_sz % PAGE));*/
		rqst->log_ptr = malloc(logptr_sz);
		assert(rqst->log_ptr);
		//assert(rqst->logptr_sz);
		//rqst->log_ptr = memalign(4096,logptr_sz);*/
	}else {
		assert(!rqst->log_ptr);
	}
#endif
...
JEMALLOC_EXPORT void *(* const __malloc_hook)(size_t size, rqst_S *) = je_malloc;
