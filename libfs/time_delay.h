...
static inline void emulate_latency_cycles(int cycles)
{
    hrtime_t start;
    hrtime_t stop;
...
static inline void emulate_pcie_latency()
{
	emulate_latency_cycles(NS2CYCLE(PCIE_LAT));
}


/*returns dealy in ns*/
static inline unsigned long calc_delay_ns(size_t datasize){
