...
int mypthread_mutex_lock(pthread_mutex_t *mutex){
return 0;
}
int mypthread_mutex_unlock(pthread_mutex_t *mutex){
return 0;
}
...
struct thpool_* thpool_init(int num_threads){
	...
	for (n=0; n<num_threads; n++){
		...
		setaffinity(&thpool_p->threads[n]->pthread);
	}
...
int thpool_add_work(thpool_* thpool_p, void *(*function_p)(void*), void* arg_p){
	...
	/* add job to queue */
	mypthread_mutex_lock(&thpool_p->jobqueue_p->rwmutex);
	jobqueue_push(thpool_p, newjob);
	mypthread_mutex_unlock(&thpool_p->jobqueue_p->rwmutex);
...
static void* thread_do(struct thread* thread_p){
	...
	mypthread_mutex_lock(&thpool_p->thcount_lock);
	thpool_p->num_threads_alive += 1;
	mypthread_mutex_unlock(&thpool_p->thcount_lock);
...
static void bsem_post(bsem *bsem_p) {
	mypthread_mutex_lock(&bsem_p->mutex);
	bsem_p->v = 1;
	pthread_cond_signal(&bsem_p->cond);
	mypthread_mutex_unlock(&bsem_p->mutex);
}
