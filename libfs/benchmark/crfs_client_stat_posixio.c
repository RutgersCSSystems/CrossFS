...
//static unsigned long runcnt = 524288;
static unsigned long runcnt = 3145728;

static int read_start = 0;
...
void* do_read(void* arg) {
	...
	int range = ops / numreader;
	int begin = thread_id*range;
	...
	for (w = 0; w < runcnt; w++) {
		if (slba >= MAX_SLBA)
			break;
                ...
                /* Wait until a write has finish */
                while (write_start == 0);

		nlb = datasize;
                /* do sequential reads */
                /* Do range read within own blocks */
                slba = (begin + w % range) * iosize;
                //slba = (w % ops) * datasize;
	...
	for (w = 0; w < runcnt; w++) {
		if (slba >= MAX_SLBA)
			break;
                ...
                /* Notify Reader when 1 write finish */
                if (w == 1)
                        write_start = 1;
		...
		nlb = datasize;
			...
			/* Do range write within own blocks */
			slba = (begin + w % range) * iosize;
                        //slba = ((w % (ops / numwriter))*numwriter+thread_id) * iosize;
                        //slba = (rand() % ops) * datasize;
                }
	...
	for (idx = 0; idx < numfs; idx++) {
		...
		r_t = malloc(numreader*sizeof(pthread_t));
                ...
                for (i = 0; i < numwriter; ++i) {
                        threadw_idx[i] = i;
                        pthread_create(&w_t[i], NULL, &do_write, &threadw_idx[i]);
                }

                for (i = 0; i < numreader; ++i) {
                        threadr_idx[i] = i;
                        pthread_create(&r_t[i], NULL, &do_read, &threadr_idx[i]);
                }
...
int getargs(int argc, char **argv) {
	...
	while (1) {
		static struct option long_options[] = {
			...
			{"pattern", required_argument,  0, 'c'},
			{0, 0, 0, 0}
		};
		/* getopt_long stores the option index here. */
		int option_index = 0;

		c = getopt_long (argc, argv, "f:j:k:r:g:d:q:w:i:s:t:u:p:v:a:b:c:",
