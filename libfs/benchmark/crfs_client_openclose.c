...
void* do_openclose(void* arg) {
	int i = 0, fd = 0, ops = 10000;
	char dirname[256];
	char fname[256];
	...
	sprintf(dirname, "/mnt/ram/%lu", pthread_self());
	mkdir(dirname, MODE);
	/* perform IO write */    
	for (i = 0; i < ops; i++) {
		sprintf(fname, "/mnt/ram/%lu/%d", pthread_self(), i % 10);
#ifdef _USE_DEVFS_CALLS
		fd = crfsopen((char *)fname, CREATDIR, MODE);
#else
		fd = open((char *)fname, CREATDIR, MODE);
#endif
                if(!fd) {
			printf("open failed \n");
			return NULL;	
	...
	for (idx = 0; idx < numfs; idx++) {
		...
		if (numreader < 0 || numwriter < 0) {
			...
			for (i = 0; i < numwriter; ++i)
				pthread_create(&w_t[i], NULL, &do_openclose, NULL);
