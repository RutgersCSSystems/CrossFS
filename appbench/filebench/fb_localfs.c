...
fb_lfs_pread(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize, off64_t fileoffset)
{
	const char* shell_crfs = NULL;
        shell_crfs = getenv("PARAFS_START");
        if (shell_crfs) {
		return (crfspread(fd->fd_num, iobuf, iosize, fileoffset));
	}
	else
...
fb_lfs_read(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize)
{
	const char* shell_crfs = NULL;
        shell_crfs = getenv("PARAFS_START");
        if (shell_crfs) {
		return (crfsread(fd->fd_num, iobuf, iosize));
	}
	else
...
fb_lfs_open(fb_fdesc_t *fd, char *path, int flags, int perms)
{
	...
	const char* shell_crfs = NULL;
        shell_crfs = getenv("PARAFS_START");
        if (shell_crfs) {
		if ((fd->fd_num = crfsopen(path, flags, perms)) < 0)
			return (FILEBENCH_ERROR);
		else
			return (FILEBENCH_OK);
...
fb_lfs_pwrite(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize, off64_t offset)
{
	const char* shell_crfs = NULL;
        shell_crfs = getenv("PARAFS_START");
        if (shell_crfs) {
		return (crfspwrite(fd->fd_num, iobuf, iosize, offset));
	}
	else
...
fb_lfs_write(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize)
{
	const char* shell_crfs = NULL;
        shell_crfs = getenv("PARAFS_START");
        if (shell_crfs) {
		return (crfswrite(fd->fd_num, iobuf, iosize));
	}
	else
