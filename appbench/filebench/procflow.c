...
procflow_wait(pid_t pid)
{
	...
	printf("master process %d waited for pid %d\n", getpid(), (int)pid);
	(void) waitpid(pid, &stat, 0);
	printf("master process %d waited 2 for pid %d\n", getpid(), (int)pid);
...
#endif
			printf("master process %d killing pid = %d\n", getpid(), pid);
			procflow_wait(pid);
			printf("master process %d finish waiting pid = %d\n", getpid(), pid);
		}
