...
#c = pdf('figure1.pdf')
c = canvas('pdf', title='result_valuesize_readrandom', dimensions=[150, 100])
t = table(file='valuesize_readrandom.data')

d = drawable(canvas=c, coord=[25,22], xrange=[-0.5,t.getmax('rownumber')+0.5],
                yrange=[0,2000], dimensions=[120, 60])

# because tics and axes are different, call axis() twice, once to
# specify x-axis, the other to specify y-axis
axis(d, linewidth=0.5, xtitle='Value sizes', xtitlesize=6,
xmanual=t.query(select='thread,rownumber'), xlabelfontsize=6,
ytitle='Throughput(kops/s)', ytitlesize=6, ylabelfontsize=6, yauto=[0,2000,500], ticmajorsize=2)

grid(drawable=d, x=False, yrange=[500,2000], ystep=500, linecolor='lightgrey',
